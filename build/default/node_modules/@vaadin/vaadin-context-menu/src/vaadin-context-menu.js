...
class ContextMenuElement extends ElementMixin(ThemePropertyMixin(GestureEventListeners(PolymerElement))) {
  static get template() {
    return html`
    <style>
      :host {
...
`;
  ...
  static get observers() {
    return ['_openedChanged(opened)', '_contextChanged(_context, _instance)', '_targetOrOpenOnChanged(listenOn, openOn)', '_templateOrRendererChanged(_contentTemplate, renderer, _context)'];
  }
  ...
  __alignOverlayPosition() {
    ...
    ['right-aligned', 'bottom-aligned'].forEach(attr => overlay.removeAttribute(attr)); // Measure actual overlay and content sizes

    const overlayRect = overlay.getBoundingClientRect();
    const contentRect = overlay.$.overlay.getBoundingClientRect(); // Maximum x and y values are imposed by content size and overlay limits.

    const xMax = overlayRect.right - contentRect.width;
    const yMax = overlayRect.bottom - contentRect.height; // Reuse saved x and y event values, in order to this method be used async
    // in the `vaadin-overlay-change` which guarantees that overlay is ready

    const x = this.__x || overlayRect.left;
    const y = this.__y || overlayRect.top; // Select one overlay corner and move to the event x/y position.
